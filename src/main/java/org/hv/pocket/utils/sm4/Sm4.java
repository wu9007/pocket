...
public class Sm4 {
    ...
    private void sm4SetKey(long[] sk, byte[] key) {
        ...
        long[] k = new long[36];
        int i = 0;
        mk[0] = getLongBe(key, 0);
        mk[1] = getLongBe(key, 4);
        mk[2] = getLongBe(key, 8);
        mk[3] = getLongBe(key, 12);
        k[0] = mk[0] ^ (long) FK[0];
        k[1] = mk[1] ^ (long) FK[1];
        k[2] = mk[2] ^ (long) FK[2];
        k[3] = mk[3] ^ (long) FK[3];
    ...
    private void sm4OneRound(long[] sk, byte[] input, byte[] output) {
        ...
        ulbuf[0] = getLongBe(input, 0);
        ulbuf[1] = getLongBe(input, 4);
        ulbuf[2] = getLongBe(input, 8);
        ulbuf[3] = getLongBe(input, 12);
        while (i < 32) {
            ulbuf[(i + 4)] = sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            i++;
        }
        putLongBe(ulbuf[35], output, 0);
        putLongBe(ulbuf[34], output, 4);
        putLongBe(ulbuf[33], output, 8);
        putLongBe(ulbuf[32], output, 12);
    }
